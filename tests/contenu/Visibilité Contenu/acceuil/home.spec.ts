...
test.describe('Visibilité - home page', () => {
    ...
    test.describe('Contenu Page', () => {
        test('Titre', async ({ page }) => {
            await expect(page.getByRole('heading', { name: 'Bienvenue chez Extia !' })).toBeVisible();
        });

        test('Bloc présentation', async ({ page }) => {
            await expect(page.getByText('Société de conseil spécialisé')).toBeVisible();
        });

        test('Lien - contact rapide - inconnu', async ({ page }) => {
            await expect(page.getByRole('button', { name: 'Qui êtes-vous ?' })).toBeVisible();
        });

        test('Lien - contact rapide - candidat', async ({ page }) => {
            await expect(page.getByRole('button', { name: 'Un candidat' })).toBeVisible();
        });

        test('Lien - contact rapide - client', async ({ page }) => {
            await expect(page.getByRole('button', { name: 'Un client' })).toBeVisible();
        });

        test('Lien - contact rapide - extien', async ({ page }) => {
            await expect(page.getByRole('button', { name: 'Un Extien' })).toBeVisible();
        });

        test('Lien - contact rapide - easter egg', async ({ page }) => {
            await expect(page.getByRole('link', { name: 'Un alien' })).toBeVisible();
        });
    });
