...
test.describe('Visibilité - Inside Extia page', () => {
    ...
    test.describe('Contenu Page', () => {
        ...
        test('Bloc - Ambition', async ({ page }) => {
            await expect(page.getByRole('heading', { name: 'L\'expérience positive' })).toBeVisible();
            await expect(page.getByText('Notre ambition est immense :')).toBeVisible();
            await expect(page.getByRole('img', { name: 'Deux collègues souriants, un' })).toBeVisible();
        });

        test('Bloc - GLOW', async ({ page }) => {
            await expect(page.getByRole('heading', { name: 'What makes us GLOW ?' })).toBeVisible();
            await expect(page.getByText('GROWLa croissance nous permet')).toBeVisible();
            await expect(page.getByText('LIBERTYEn donnant à chaque')).toBeVisible();
            await expect(page.getByText('OPERATE Nous ne sommes pas')).toBeVisible();
            await expect(page.getByText('WORKNotre aventure est')).toBeVisible();
        });

        test('Bloc - Manifeste', async ({ page }) => {
            page.getByRole('heading', { name: 'Manifeste Extia' }).scrollIntoViewIfNeeded();
        ...
        test('Bloc - Aprentissage', async ({ page }) => {
            await expect(page.getByRole('heading', { name: 'L’entreprise apprenante' })).toBeVisible();
            await expect(page.getByText('Toutes les occasions sont')).toBeVisible();
            await expect(page.getByRole('heading', { name: 'Notre modèle d’apprentissage' })).toBeVisible();
            await expect(page.getByText('70Nous apprenons à 70% en')).toBeVisible();
            await expect(page.getByText('20Nous apprenons à 20% en é')).toBeVisible();
            await expect(page.getByText('10Nous apprenons à 10% via la')).toBeVisible();
        });

        test('Lien - En savoir plus sur l\'entreprise', async ({ page }) => {
            await expect(page.getByRole('link', { name: 'En savoir plus sur notre' })).toBeVisible();
        });

        test('Bloc - Great Place To', async ({ page }) => {
            await expect(page.getByRole('heading', { name: 'Great place to Work Live' })).toBeVisible();
            await expect(page.locator('.GreatPlaceToWork__Wrapper-sc-1rhbsol-0 > div:nth-child(2)')).toBeVisible();
            await expect(page.getByText('Se créer des souvenirs communs250. C’est le nombre d’événements festifs,')).toBeVisible();
            await expect(page.getByText('What you measure is what you get !La satisfaction des Extiens est')).toBeVisible();
            await expect(page.getByText('Une approche « collaborActive »“Venez comme vous êtes”. Certes, cette citation')).toBeVisible();
        });

        test('Lien - En savoir plus sur le label', async ({ page }) => {
            await expect(page.getByRole('link', { name: 'En savoir plus sur le label' })).toBeVisible();
        });

        test('Bloc - Actu', async ({ page }) => {
            await expect(page.getByRole('heading', { name: 'Nos dernières actualités' })).toBeVisible();
            await expect(page.locator('.sc-gsTEea.hfvcqA.news--slider > .sc-dlfnuX > .swiper-container > .swiper-wrapper')).toBeVisible();
